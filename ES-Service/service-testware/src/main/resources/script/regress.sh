...
export BEA_ROOT=/home/entitlem/bea_config_wls10/applications
...
if [[ "$INSTANCE" != "" ]]
then
   export SERVER_MAIN_LOGS=$BEA_ROOT/${INSTANCE}/log
   export SERVER_APP_LOGS=$BEA_ROOT/${INSTANCE}/$APP_NAME/log
fi

if [[ ! -d "$SERVER_MAIN_LOGS" ]]
then
   echo "Instance log dir does not exist: $SERVER_MAIN_LOGS !"
   exit 1
fi
...
start_date=`date`
print "Regression started $start_date for ${ST_VIEW} ${INSTANCE}" | mail -s"SERVICE MAVEN regression started ${ST_VIEW} ${INSTANCE}" ${MAILTO}
...
# new dir name
export REVIEW_DIR="$HOME/testreview/t$(date +%Y%m%d_%H%M)_svc_$NAME"
...
then
   ...
   mkdir -p $REVIEW_DIR/server_logs
fi
...
##############################################################
############### Copy server logs if requested ################

if [[ "$INSTANCE" != "" ]]
then
   echo "+++ Copying server logs..."
   cp -R ${SERVER_MAIN_LOGS}/* $REVIEW_DIR/server_logs
   if [[ -d ${SERVER_APP_LOGS} ]]
   then
      cp -R ${SERVER_APP_LOGS}/* $REVIEW_DIR/server_logs
   fi
fi
...
ST_PROJECT_URL:        $ST_PROJECT_URL

NAME:              $NAME
TEST_HOME:      $TEST_HOME

BEA INSTANCE:      $INSTANCE
SERVER_MAIN_LOGS:  $SERVER_MAIN_LOGS
SERVER_APP_LOGS:   $SERVER_APP_LOGS
================================================================================
