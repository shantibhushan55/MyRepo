 ...
 *
 */
package com.hp.es.service.unitEventHistory;
...
import com.hp.es.service.unitEventHistory.orchestration.UnitEventHistoryOrchestraction;
...
public class GetUnitEventHistoryOperation extends EsOperation {
    ...
    protected EsReply execute(EsRequestComplexType request, OperationContext context,MetricsHandler metricsHandler)
        ...
        try {
            ESLog.debug("Calling UnitEventHistory  orchestration ...");
            return UnitEventHistoryOrchestraction.getInstance().execute(request, context, metricsHandler);
        } finally {
            ESLog.debug("Exit");
        }
    }
    ...
    protected void verifyRequest(EsRequestComplexType request, MetricsHandler metricsHandler)
        ...
        if( (unitHistoryRequest.getProductNumber()!=null) && (unitHistoryRequest.getProductNumber().trim().length()!=0)){
			int maximum=Configuration.getInstance().getProperties().getIntegerProperty(EsConstants.ES_PN_MAXLENGTH,20);
			ProductNumberValidator.verifyProductNumberGreaterThan(unitHistoryRequest.getProductNumber(), maximum);
		}
        /*
